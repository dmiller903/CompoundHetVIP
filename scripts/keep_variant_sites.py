...
each sample file and removes all non-variant sites. If parental files are included, the script then filters each parent \
file for sites that only occur in the affected sample of that family.')

parser.add_argument('sample_gVCF', help='Sample File')
...
if parent1File != None and parent2File != None:
    ...
    timeElapsedMinutes = round((time.time()-startTime) / 60, 2)
    timeElapsedHours = round(timeElapsedMinutes / 60, 2)
    print('Sites not corresponding to sample file have been removed for each parent. Time elapsed: {} minutes ({} hours)'.format(timeElapsedMinutes, timeElapsedHours))
print("Done!")