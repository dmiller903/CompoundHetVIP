...
scores and gene lengths (both the total gene length and CDS gene length) to a file output from "identify_CH_variants.py", \
"identify_homAlt_variants.py", or "identify_deNovo_variants.py". If working with controlled data, this script can also \
anonymize family ID and sample ID information.')

parser.add_argument('variant_file', help='File generated by GEMINI query')
...
chDict = {}
with open(variantFile) as queryFile:
...
with open(variantFile) as queryFile, open("/tmp/new_query.txt", 'w') as newQueryFile:
    header = queryFile.readline()
    headerList = header.rstrip("\n").split("\t")
    if "family_genotypes" in header:
        sampleIndex = headerList.index("samples")
